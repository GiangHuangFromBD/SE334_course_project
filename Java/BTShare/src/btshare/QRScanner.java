...
public class QRScanner {
    ...
    if (decodedQR.equals("")) AlertDialogWorker.showAlertDialog("invalidqr", stage); 
    
    if (decodedQR.charAt(0) != '1'  && decodedQR.charAt(0) != '2'     )
        AlertDialogWorker.showAlertDialog("invalidqr", stage); 
    ...
    private static void handleFileSendRequest(String request,Stage stage) {  // REQUESTCODE = 2   "2  AB:78:99:1F:66:0E  programming_java7e.pdf  7890000  UUIDs
        ...
        if ( requestArray.length <  5) {  AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;    }  /// NOT ENOUGH INFO TO LOAD THE FILE
        if ( !isAdded(requestArray)) { AlertDialogWorker.showAlertDialog("notadded", stage);  return; }
        if ( !requestArray[2].contains(".")  ) {AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;}     ////  UNKNOWN FILE TYPE, NO DOT FOUND
        int fileSize = 0;
        try { fileSize = Integer.parseInt(requestArray[3]); } catch (NumberFormatException nfe) {  AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;  }
                                                                                                /// INVALID FILESIZE
        if (fileSize <= 0) {  AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;  } /// MUST BE POSITIVE
        if (!ProcessorController.isBusy() ) showAcceptFileDialog(requestArray,stage);
        else AlertDialogWorker.showAlertDialog("busy", stage); 
    }
    ...
    private static boolean isAdded(String[] requestArray) {
    AtomicBoolean hasBonded = new AtomicBoolean();
        CountDownLatch latch = new CountDownLatch(1);
        Runnable checkJob = () -> {
            hasBonded.set( DatabaseWorker.existsDevice(requestArray[1]) );
            latch.countDown();
        };
    ...
    private static void showAddDeviceDialog(String[] requestArray, Stage stage) {         
            ...
            else AlertDialogWorker.showAlertDialog("off", stage); 
    }
    ...
    private static void handleAddRequest(String request, Stage stage) {// REQUESTCODE = 1    "1  AB:78:99:1F:66:0E  4"
      String[] requestArray = request.split("  ");
        if ( requestArray.length <  3) {  AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;  }
        if (!isDeviceBonded(requestArray[1].replace(":", "")))  { AlertDialogWorker.showAlertDialog("unbonded", stage); 
       // try { BluetoothBonding.discoverAndBondNewDevice(requestArray[1].replace(":", "")); } catch (Exception e) { e.printStackTrace(); }        
        return;     }
        if(!isValidString(requestArray[1]))  { AlertDialogWorker.showAlertDialog("invalidqr", stage);   return; }
        
        int cores = 0;
        try { cores = Integer.parseInt(requestArray[2]); } catch (NumberFormatException nfe) {  AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;  }
        if (cores <= 0) {  AlertDialogWorker.showAlertDialog("invalidqr", stage);  return;  } /// MUST BE POSITIVE
